...
static PFSection *xpf_section_for_vmaddr(uint64_t vmaddr)
{
	PFSection *candidates[] = {
		gXPF.kernelBootTextSection,
		gXPF.kernelTextSection,
		NULL,
	};
	for (int i = 0; candidates[i]; i++) {
		PFSection *section = candidates[i];
		if (vmaddr >= section->info.vmaddr && vmaddr < (section->info.vmaddr + section->info.size)) {
			return section;
		}
	}
...
static bool xpf_vmaddr_in_kernel_data(uint64_t vmaddr)
{
	PFSection *dataSections[] = {
		gXPF.kernelDataConstSection,
		gXPF.kernelConstSection,
		gXPF.kernelDataSection,
		NULL,
	};
	for (int i = 0; dataSections[i]; i++) {
		PFSection *section = dataSections[i];
		if (vmaddr >= section->info.vmaddr && vmaddr < (section->info.vmaddr + section->info.size)) {
			return true;
		}
	}
...
static uint64_t xpf_find_arm_vm_init_darwin25(void)
{
	uint64_t start_first_cpu = xpf_item_resolve("kernelSymbol.start_first_cpu");
	PFSection *bootSection = gXPF.kernelBootTextSection;
	XPF_ASSERT(bootSection);

	uint64_t sectionEnd = bootSection->info.vmaddr + bootSection->info.size;
	for (uint64_t addr = start_first_cpu; addr + 4 < sectionEnd && addr < start_first_cpu + 0x400; addr += 4) {
		uint32_t inst = pfsec_read32(bootSection, addr);
		uint64_t target = 0;
		bool isBl = false;
		if (arm64_dec_b_l(inst, addr, &target, &isBl) != 0 || !isBl) continue;
		if (target < gXPF.kernelTextSection->info.vmaddr) continue;

		uint64_t fn = pfsec_find_function_start(gXPF.kernelTextSection, target);
		if (fn) return fn;
	}
	return 0;
}
...
{
	if (gXPF.darwinVersion && strcmp(gXPF.darwinVersion, "25.0.0") >= 0) {
		uint64_t arm_vm_init = xpf_find_arm_vm_init_darwin25();
...
static uint64_t xpf_find_arm_vm_init_reference(uint32_t n)
{
	if (gXPF.darwinVersion && strcmp(gXPF.darwinVersion, "25.0.0") >= 0) {
		...
		if (n == 1) {
			return pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(bootSection, start_first_cpu + 0xd8);
		}
		if (n == 2) {
			return pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(bootSection, start_first_cpu + 0xe0);
		}
		if (n == 5) {
			uint64_t gPhysSize = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(bootSection, start_first_cpu + 0x4bc);
			if (gPhysSize) return gPhysSize;
		}
	}
...
{
	PFSection *textSection = gXPF.kernelPPLTextSection;
	if (!textSection) {
...
static uint64_t xpf_find_phystokv(void)
{
	...
	uint32_t blIndices[] = {
		gXPF.kernelBase == 0xfffffe0007004000 ? 2 : 1,
		1, 2, 3, 4,
	};
	uint32_t blIndexCount = (gXPF.darwinVersion && strcmp(gXPF.darwinVersion, "25.0.0") >= 0) ? 5 : 1;

	for (uint32_t j = 0; j < blIndexCount; j++) {
		uint32_t n = blIndices[j];
		...
		uint64_t phystokv = 0;
		if (arm64_dec_b_l(pfsec_read32(gXPF.kernelTextSection, blAddr), blAddr, &phystokv, NULL) == 0 && phystokv) {
			if (pfsec_find_function_start(gXPF.kernelTextSection, phystokv)) {
				return phystokv;
			}
		}
	}
...
static uint64_t xpf_find_ptov_table(void)
{
	...
	uint64_t ldrAddr = 0;
	uint32_t maxLdr = (gXPF.darwinVersion && strcmp(gXPF.darwinVersion, "25.0.0") >= 0) ? 6 : 2;
	for (uint32_t ldrIdx = 0; ldrIdx < maxLdr; ldrIdx++) {
		ldrAddr = pfsec_find_next_inst(gXPF.kernelTextSection, toCheck, 0x80, ldrAny, ldrAnyMask);
		if (!ldrAddr) break;
		uint64_t ptov_table = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, ldrAddr);
		if (ptov_table && xpf_vmaddr_in_kernel_data(ptov_table)) {
			return ptov_table;
		}
		toCheck = ldrAddr + 4;
	}
...
{
	PFSection *entrySection = xpf_section_for_vmaddr(gXPF.kernelEntry);
	uint64_t start_first_cpu = 0;

	for (uint64_t off = 0; off < 0x80; off += 4) {
		uint64_t addr = gXPF.kernelEntry + off;
		uint32_t inst = pfsec_read32(entrySection, addr);
		if (arm64_dec_b_l(inst, addr, &start_first_cpu, NULL) == 0 && start_first_cpu) {
...
static uint64_t xpf_find_cpu_ttep_darwin25(PFSection *bootSection, uint64_t start_first_cpu)
{
	uint64_t sectionEnd = bootSection->info.vmaddr + bootSection->info.size;
	for (uint64_t addr = start_first_cpu; addr + 0x30 < sectionEnd; addr += 4) {
		uint32_t inst = pfsec_read32(bootSection, addr);
		bool isCbnz = false;
		arm64_register reg;
		if (arm64_dec_cb_n_z(inst, addr, &isCbnz, &reg, NULL) != 0 || isCbnz) continue;

		uint64_t cpu_ttep = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(bootSection, addr + 0x1c);
		if (cpu_ttep && xpf_vmaddr_in_kernel_data(cpu_ttep)) {
			return cpu_ttep;
		}
	}
	return 0;
...
static uint64_t xpf_find_cpu_ttep(void)
{
	...
	PFSection *bootSection = xpf_section_for_vmaddr(start_first_cpu);

	if (gXPF.darwinVersion && strcmp(gXPF.darwinVersion, "25.0.0") >= 0) {
		uint64_t cpu_ttep = xpf_find_cpu_ttep_darwin25(bootSection, start_first_cpu);
		XPF_ASSERT(cpu_ttep);
		return cpu_ttep;
	}
...
static uint64_t xpf_find_kernel_el(void)
{
	...
	uint32_t inst = pfsec_read32(bootSection, start_first_cpu + 16);
