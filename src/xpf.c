...
int xpf_start_with_kernel_path(const char *kernelPath)
{
	...
	if (LITTLE_TO_HOST(*(uint32_t *)(gXPF.mappedKernel)) == MH_MAGIC_64
		...
		if (gXPF.decompressedKernel) {
			/* Drop compressed mapping — peak RAM was ~2× kernel size and jetsam'd Dopamine during patchfind. */
			munmap(gXPF.mappedKernel, gXPF.kernelSize);
			gXPF.mappedKernel = NULL;
	...
	gXPF.kernelBootTextSection = xpf_pfsec_init("com.apple.kernel", "__TEXT_BOOT_EXEC", NULL);
...
int xpf_offset_dictionary_add_set_by_name(xpc_object_t xdict, const char *name)
{
	if (!name || !xdict)
		return -1;
	bool foundUnsupported = false;
	XPFSet *set = xpf_find_set(name, &foundUnsupported);
	if (set)
		return xpf_offset_dictionary_add_set(xdict, set);
	if (foundUnsupported) {
		xpf_set_error("Set \"%s\" is unsupported by this device", name);
		return -1;
	}
	xpf_set_error("Failed to find set \"%s\"", name);
	return -1;
}
