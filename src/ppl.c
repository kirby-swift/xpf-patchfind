...
static PFSection *xpf_ppl_text_section(void)
{
	return gXPF.kernelPPLTextSection ? gXPF.kernelPPLTextSection : gXPF.kernelTextSection;
}

static bool xpf_darwin25_no_ppltext(void)
{
	return gXPF.kernelIsArm64e
		&& !gXPF.kernelPPLTextSection
		&& gXPF.darwinVersion
		&& strcmp(gXPF.darwinVersion, "25.0.0") >= 0;
}

static uint64_t xpf_find_function_via_string_xref(const char *string, uint32_t xrefMask)
{
	...
	PFSection *textSection = xpf_ppl_text_section();
	PFXrefMetric *xrefMetric = pfmetric_xref_init(stringAddr, xrefMask);
	__block uint64_t functionAddr = 0;
	pfmetric_run(textSection, xrefMetric, ^(uint64_t vmaddr, bool *stop) {
		functionAddr = pfsec_find_function_start(textSection, vmaddr);
		*stop = true;
	});
	pfmetric_free(xrefMetric);
	XPF_ASSERT(functionAddr);

	return functionAddr;
}

static uint64_t xpf_find_vm_page_bootstrap_and_addr(void)
{
	uint32_t inst = 0x120a6d28;
...
static uint64_t xpf_find_vm_page_bootstrap_reference(uint32_t idx)
{
	uint64_t andAddr = xpf_find_vm_page_bootstrap_and_addr();
	XPF_ASSERT(andAddr);

	uint64_t bootstrapFunc = pfsec_find_function_start(gXPF.kernelTextSection, andAddr);
	XPF_ASSERT(bootstrapFunc);
	...
	#define VM_BOOTSTRAP_UNIQUE_MAX 16
	__block uint64_t *unique = calloc(VM_BOOTSTRAP_UNIQUE_MAX, sizeof(uint64_t));
	__block uint32_t uniqueCount = 0;

	PFPatternMetric *metric = pfmetric_pattern_init(&ldrAny, &ldrAnyMask, sizeof(ldrAny), sizeof(uint32_t));
	pfmetric_run_in_range(gXPF.kernelTextSection, bootstrapFunc, bootstrapFunc + 0x3000, metric, ^(uint64_t vmaddr, bool *stop) {
		uint64_t sym = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, vmaddr);
		if (!sym || !pfsec_contains_vmaddr(gXPF.kernelDataConstSection, sym)) {
			return;
		}
		...
		if (uniqueCount < VM_BOOTSTRAP_UNIQUE_MAX) {
			unique[uniqueCount++] = sym;
		}
	});
	pfmetric_free(metric);

	XPF_ASSERT(idx < uniqueCount);
	uint64_t result = unique[idx];
	free(unique);
	return result;
}

static uint64_t xpf_find_pmap_enter_options_addr_no_ppltext(void)
{
	return xpf_find_function_via_string_xref("pmap_enter_options() pmap %p v 0x%llx @%s:%d", XREF_TYPE_MASK_REFERENCE);
}

static uint64_t xpf_find_pmap_remove_options_no_ppltext(void)
{
	return xpf_find_function_via_string_xref("pmap_remove_range_options", XREF_TYPE_MASK_REFERENCE);
}
...
static bool xpf_is_kernel_data_vmaddr(uint64_t vmaddr)
{
	return pfsec_contains_vmaddr(gXPF.kernelDataConstSection, vmaddr);
}

static bool xpf_symbol_touched_in_range(uint64_t sym, uint64_t from, uint32_t byteRange)
{
	for (uint64_t pc = from; pc < from + byteRange; pc += 4) {
		if (pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, pc) == sym) {
			return true;
		}
	}
	return false;
...
// iOS 26+ (Darwin 25.x): __PPLTEXT is gone; locate trust cache runtime via trust_cache_init.
static uint64_t xpf_find_ppl_trust_cache_rt_no_ppltext(void)
{
	PFStringMetric *stringMetric = pfmetric_string_init("trust_cache_init");
	XPF_ASSERT(stringMetric);

	__block uint64_t trustCacheInitString = 0;
	pfmetric_run(gXPF.kernelStringSection, stringMetric, ^(uint64_t vmaddr, bool *stop) {
		trustCacheInitString = vmaddr;
		*stop = true;
	});
	pfmetric_free(stringMetric);
	XPF_ASSERT(trustCacheInitString);

	PFXrefMetric *xrefMetric = pfmetric_xref_init(trustCacheInitString, XREF_TYPE_MASK_REFERENCE);
	__block uint64_t trustCacheInit = 0;
	pfmetric_run(gXPF.kernelTextSection, xrefMetric, ^(uint64_t vmaddr, bool *stop) {
		trustCacheInit = pfsec_find_function_start(gXPF.kernelTextSection, vmaddr);
		*stop = true;
	});
	pfmetric_free(xrefMetric);
	XPF_ASSERT(trustCacheInit);

	uint32_t adrpInst = 0, adrpInstAny = 0;
	arm64_gen_adr_p(OPT_BOOL(true), OPT_UINT64_NONE, OPT_UINT64_NONE, ARM64_REG_ANY, &adrpInst, &adrpInstAny);

	uint64_t toCheck = trustCacheInit;
	uint64_t candidate = 0;
	for (int i = 0; i < 64; i++) {
		uint64_t adrpAddr = pfsec_find_next_inst(gXPF.kernelTextSection, toCheck, 400, adrpInst, adrpInstAny);
		if (!adrpAddr) break;
		toCheck = adrpAddr + 4;

		uint64_t sym = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, adrpAddr + 4);
		if (!xpf_is_kernel_data_vmaddr(sym)) continue;
		if (!xpf_symbol_touched_in_range(sym, adrpAddr, 0x40)) continue;

		candidate = sym;
		break;
	}
	XPF_ASSERT(candidate);
	return candidate;
}
...
static uint64_t xpf_find_pmap_enter_pv(void)
{
	...
	pfmetric_run(textSection, xrefMetric, ^(uint64_t vmaddr, bool *stop) {
		pmap_enter_pv = pfsec_find_function_start(textSection, vmaddr);
		*stop = true;
	});
	pfmetric_free(xrefMetric);
...
static uint64_t xpf_find_pv_head_table(void)
{
	...
	uint64_t toCheck = pmap_enter_pv;
	for (int i = 0; i < 64; i++) {
		uint64_t ref = pfsec_find_next_inst(textSection, toCheck, 200, ldrAnyInst, ldrAnyMask);
		if (!ref) break;
		uint64_t sym = pfsec_arm64_resolve_adrp_ldr_str_add_reference_auto(gXPF.kernelTextSection, ref);
		if (sym) {
			return sym;
		}
		toCheck = ref + 4;
...
void xpf_ppl_init(void)
{
	...
	if (xpf_darwin25_no_ppltext()) {
		...
		xpf_item_register("kernelSymbol.pmap_enter_options_addr", xpf_find_pmap_enter_options_addr_no_ppltext, NULL);
		xpf_item_register("kernelSymbol.pmap_remove_options", xpf_find_pmap_remove_options_no_ppltext, NULL);
		xpf_item_register("kernelSymbol.vm_first_phys", xpf_find_vm_page_bootstrap_reference, (void *)(uint32_t)0);
		xpf_item_register("kernelSymbol.vm_last_phys", xpf_find_vm_page_bootstrap_reference, (void *)(uint32_t)1);
		xpf_item_register("kernelSymbol.pp_attr_table", xpf_find_vm_page_bootstrap_reference, (void *)(uint32_t)5);

		if (strcmp(gXPF.darwinVersion, "22.0.0") >= 0) {
			xpf_item_register("kernelSymbol.ppl_trust_cache_rt", xpf_find_ppl_trust_cache_rt_no_ppltext, NULL);
		}
		else {
